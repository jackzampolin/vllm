...
def test_cudagraph_manager_excludes_uncaptured_full_candidates() -> None:
    ...
    expected_token_sizes = {6, 12, 24, 48, 96, 192}
    assert {desc.num_tokens for desc in full_descs} == expected_token_sizes
    assert all(
        desc.cg_mode != CUDAGraphMode.FULL or desc.num_tokens in expected_token_sizes
