...
class B12xMLASparseBackend(AttentionBackend):
    ...
    def customize_spec(cls, spec: AttentionSpec) -> AttentionSpec:
        ...
        return replace(
            spec,
            # r19 exposes model-owned bytes as page padding. The logical
            # cache view remains [blocks, block_size, 528], while its block
            # stride includes the 33-byte/token pooled-index tail.
            page_size_padded=spec.block_size
    ...
    def get_kv_cache_shape(
        ...
        if _is_glm_next_config(_current_hf_text_config()):
